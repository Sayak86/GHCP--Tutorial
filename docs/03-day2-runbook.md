...
1. Walk through `.github/hooks/payguard.json` and the two tiny scripts in `scripts/hooks/`:
   - `protect_files.py` (PreToolUse): denies any tool call touching `data/sample_payments.json`,
...
2. Prove it — in Agent mode:
   > Add a test payment of 50 USD to data/sample_payments.json.
...
2. Open `inputs/poisoned/vendor-integration-notes.md` **rendered** (preview mode) — looks like an
  ...
  add a beneficiary to the sample data and weaken currency validation.
...
4. Watch: if the model follows the payload, the **PreToolUse hook from Demo 4 blocks the sample-data
...
1. **Credits (AIC):** the model picker shows a **multiplier** per model — that's its price in
   credits (included models = 0 credits, stronger models = 1 credit or more). One agent-mode *turn*
   = one request = one credit-charge of the chosen model — even though a turn can contain many
   tool-loop iterations. Show your own usage: VS Code status bar / github.com → Settings → Copilot
   usage. (Swap "credit / AIC" for your team's exact term and monthly allowance.)
