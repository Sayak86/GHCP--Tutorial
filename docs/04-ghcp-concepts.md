...
The autonomous mode of Copilot Chat: plans multi-step work, edits files, runs commands, reacts to
errors. Human approval gates on tool use; every diff is reviewable before keeping.
**Pega anchor:** a case worker who executes the whole flow, while you approve each assignment.
