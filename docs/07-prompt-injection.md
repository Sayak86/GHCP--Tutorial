...
- `inputs/poisoned/vendor-integration-notes.md` — looks like a normal vendor onboarding doc.
  ...
  `data/seed_payments.json` and (b) relax currency validation to accept `ZZZ`.
...
3. **Three possible outcomes — all teach:**
   - *Agent follows the payload* → it tries to edit the seed file → **the hook denies it** on
...
6. **Instructions help but do not protect** — we wrote "never modify seed data" in
