...
- Simplest mental model of "the agent": it works in a **loop** — *you give it a task → it decides a
  step → it does the step (reads a file, edits, runs a test) → it looks at the result → it decides
  the next step → …* until the job is done or it needs you. Everything we customize over two days —
  instructions, prompts, agents, skills, hooks — just changes what goes **into** that loop or
  **checks** what comes out. (The formal name for this loop is the "harness" — you don't need the
  word to teach it.)
- Pega hook for the room: this loop is like a **case lifecycle engine**; instructions are your
  **rulesets**; hooks are your **guardrails**; agents are **personas/portals**.
- The chat modes, simplest first:
  - **Ask** — answers your question, touches nothing (a chatbot).
  - **Plan** — writes a step-by-step *plan* for a change, but does **not** edit anything. The recipe.
  - **Agent** — actually does the work: edits, runs, fixes, repeats — with your approval gates. The cook.
  - (**Edit** — a middle option: proposes edits to files you pick, no autonomy.)
  - The habit to preach: for anything big, **Plan first, read the recipe, then let Agent cook.**
- The **model picker**: different models, different strengths, different *credit cost* (the
  multiplier). We will come back to credits (AIC) on Day 2.

**Do**
1. Open Copilot Chat. Show the mode dropdown (Ask / Plan / Edit / Agent) and the model picker.
2. In **Ask** mode:
...
3. Now build the instructions file **live with the room** — this is the "design together" moment.
   ...
   - Never modify `data/sample_payments.json` or any file under `inputs/`.
...
1. In Agent mode:
   ...
   > Structure: app/main.py (FastAPI app + endpoints), app/store.py (in-memory store loaded from
   > data/sample_payments.json), tests/test_payments.py. Create a small data file with 5 example payments.
