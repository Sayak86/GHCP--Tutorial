...
- No database. Storage is an in-memory dictionary in `app/store.py`, seeded from
  `data/seed_payments.json`.
...
- Never modify `data/seed_payments.json` or anything under `inputs/` — these are controlled
  training fixtures.
